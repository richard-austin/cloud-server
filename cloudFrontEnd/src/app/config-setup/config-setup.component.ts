...
export class ConfigSetupComponent implements CanComponentDeactivate, OnInit, AfterViewInit, OnDestroy {
  ...
  setPTZControlsCheckboxDisabledState(index: number): boolean {
    ...
    if (ovhc.value == '') {
      ptzc.setValue(false);
    }  // Ensure PTZ is set to "off" if onvifHost has the (valid) value empty
  ...
  getCameraParamSpecsReferenceCopy(camera: Camera): CameraParamSpec {
    if (camera?.cameraParamSpecs?.camType === undefined) {
      return this.cameraSvc.cameraParamSpecs[0];
    }// Return the Not Listed option
    else {
    ...
    const toCameraGroups = this.list$.value.map((camera) => {
      ...
      const toStreamGroups = streamList$.value.map((stream: Stream) => {
        return new UntypedFormGroup({
          ...
          audio_encoding: new UntypedFormControl({
            value: stream.audio_encoding,
            disabled: !stream.audio
          }, [Validators.required, Validators.pattern(/^(AAC|G711|)$/)]),
          audio_sample_rate: new UntypedFormControl({
            value: stream.audio_sample_rate,
            disabled: !stream.audio || stream.audio_encoding === 'None'
  ...
  toggleRecordingSetupDialogue(camId: string) {
    ...
    if (this.camForRecordingSetup === camId) {
      this.recordingSetupComponents.forEach((r) => {
        if (r.camKey === camId) {
          r.setupData();
        }  // Reload recording setup component from main config data
      });
    }
  }
